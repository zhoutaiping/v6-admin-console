<template>
  <span>
    <transition :name="transition" @after-enter="handleAfterEnter" @after-leave="handleAfterLeave">
      <div v-show="!disabled && showPopper" ref="popper" :class="[popperClass, content && 'yd-popover--plain']" :style="{ width: width + 'px' }" :id="tooltipId" :aria-hidden="(disabled || !showPopper) ? 'true' : 'false'" class="yd-popover yd-popper" role="tooltip">
        <div v-if="title" class="yd-popover__title" v-text="title"/>
        <slot>{{ content }}</slot>
      </div>
    </transition>
    <slot name="reference"/>
