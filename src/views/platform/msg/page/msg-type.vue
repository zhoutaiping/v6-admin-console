<template>
  ...
  >
    <div slot="filter">
      <el-button type="primary" @click="$refs.addedit.handleOpen(null,'Create')">新 增</el-button>
    ...
    >
      ...
      >
        <template slot-scope="{row}">
          <router-link :to="`/msg/type/${row.id}`">
            <el-button
              type="text"
              @click="e =>{}"
            >管理</el-button>
...
import AddEdit from '../components/add-edit-type'
export default {
  components: { AddEdit },
  mixins: [consoleData],
  data() {
    return {
      ...
      column: [
        ...
        { label: '消息类型', dataIndex: 'name', cell: 'name' },
        { label: '标识', dataIndex: 'desc' },
        { label: '属性', dataIndex: 'params' },
        { label: '状态', dataIndex: 'status' },
        { label: '添加时间', dataIndex: 'created_at' }
      ],
      options: {
        checkRow: []
      }
