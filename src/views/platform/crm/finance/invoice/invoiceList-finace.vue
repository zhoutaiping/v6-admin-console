...
<template>
  <div class="app-container-finance-invoice-list">
    <DmData ref="DmData" @init="fetchList">
      ...
      <el-tabs v-model="tabs_name" type="card" class="tab-status tab-box" @tab-click="handleTabsClick">
        <el-tab-pane v-for="(item,index) in TABS" :key="index" :name="item.value" :label="item.label"/>
      </el-tabs>
      <DisposeTab v-loading="loading" v-if="tabs_name === 'doing'" :list="list" @handleload="$refs.DmData.initPage()"/>
      <SuccessTab v-loading="loading" v-else :list="list" @handleload="$refs.DmData.initPage()"/>
...
export default {
  ...
  data() {
    return {
      ...
      tabs_name: 'doing',
      TABS: [{ label: '待处理发票 ', value: 'doing' }, { label: '已处理发票', value: 'done' }],
  ...
  computed: {
    API_INDEX() {
      if (this.tabs_name === 'doing') return '/fd/V4/invoice.list.doing'
      if (this.tabs_name === 'done') return '/fd/V4/invoice.list.done'
    }
  },
  watch: {
    responseData: {
