        ...
        <DmAlert>
          <el-form label-position="left" label-width="120px">
            <el-form-item v-for="i in info" :key="i.label" :label="i.label" class="item-box">
              {{ formartValue(disp_ippool_info, i.value) }}
            </el-form-item>
          </el-form>
        </DmAlert>
...
export default {
  ...
  computed: {
    ...mapState({
      disp_ippool_info: state => state.disp.disp_ippool_info,
  ...
  methods: {
    formartValue(data, prop) {
      ...
      if (prop === 'service_type') {
        return this.options.service_type.find(e => { return e.value === value }) ? this.options.service_type.find(e => { return e.value === value }).label : value
      } else if (prop === 'node_type') {
        return this.node_type.find(e => { return e.value === value }) ? this.node_type.find(e => { return e.value === value }).label : value
      } else if (prop === 'cname_server_slaves') {
        return value && Array.isArray(value) && value.join(',')
      } else if (prop === 'group_name') {
        return value && Array.isArray(value) && value.join(',')
      }
      return value || '暂无'
    },
    ...
    handleSubmit() {
      this.$refs.Form.validate(async(valid) => {
        if (valid) {
          ...
          await this.Fetch.put('/fd/V4/disp.ippool.batch.remark', params)
          await this.$store.dispatch('fetchDispIppoolInfo', { disp_ippool_id: this.$route.params.id })
