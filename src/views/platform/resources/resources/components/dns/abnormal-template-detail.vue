<template>
  <console-page-layout>
    ...
    <DmToolbar>
      <el-button
        type="primary"
        @click="$refs.DialogTemplateRule.handleOpen()"
      >新增规则</el-button>
      <el-button
        ...
        >
          ...
          >
            <template slot-scope="scope">
              <ColumnBtnSort
                ...
                type="text"
                @on-set-up="handleRowSort(scope.row, 'top')"
                @on-up="handleRowSort(scope.row, 'prev')"
                @on-down="handleRowSort(scope.row, 'next')"
                @on-set-down="handleRowSort(scope.row, 'bottom')"
              />
            </template>
          </el-table-column>
          <el-table-column
          ...
          >
            <template slot-scope="scope">
              <el-button
                type="text"
                @click="$refs.DialogTemplateRule.handleOpen(scope.row)"
              >编辑</el-button>
            </template>
          </el-table-column>
        </el-table>
...
import DialogTemplateRule from './DialogTemplate/DialogTemplateAbnormalRule'
import ColumnRules from './ColumnRules'

export default {
  components: { PopoverInput, DialogTemplateRule, ColumnRules, ColumnBtnSort },
  ...
  created() {
    this.$store.dispatch('fetchSettings', { type: 'abnormal', template_id: this.$route.params.id || '' })
  },
  methods: {
    // 应用模板
    async handleRowUse() {
    ...
    async handleRowSort(row, type) {
      const form = {
        id: row.id,
        template_id: this.$route.params.id,
        ...this.$refs.DmData.getPagination(),
        type
      }
      try {
        await this.Fetch.post('/fd/V4/dispatch.rule.sort.save', form)
      } catch (e) {
        return
      }
      this.Message('ACTION_SUCCESS')
